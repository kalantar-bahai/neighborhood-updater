...
**`nucleusType` — live on your side, deliberately held back on ours, 2026-09-13.** You exposed `nucleusType` (query + patch), but confirmed it always reads "Neighborhood" when `location` is set (true for all 7 of your currently seeded nuclei), overriding whatever's actually stored — only settable/meaningful when `location` is unset, a path not exercisable in your current data. Our picker (`AppClient.tsx`) groups nuclei into Neighborhood/Network/Population cards by this exact field. Migrating now would silently miscategorize any real Network/Population nucleus as Neighborhood — a correctness regression, not just blank/stale display like the earlier cluster/grouping/pg case. Decided with the user: holding off until this is safe to exercise (either your location-override behavior changes, or we can verify no currently-Network/Population nucleus would be affected). Still sheet-sourced on our side.
